  ...
  private def gen_Class_Header(): Unit = {
    classContent.append(
      s"\n\n/*\n * class implementing the FK - Referential Integrity - Invariant\n */" +
        s"\nclass ${pk_table_names._4}[Time, PK](" +
        s"\n\t\t\t\t\t${pk_table_names._3}: ${pk_table_names._2}[Time, PK]," +
        fk_Tables_names.map(
          t_Names => s"\n\t\t\t\t\t${t_Names._3}: ${t_Names._2}[Time, PK]"
        ).mkString(", ") + s" )" +
        s"\n\t\t\t\t\textends CvRDT[${pk_table_names._4}[Time, PK]] {"
    )
  }
  ...
  private def gen_Compatible(): Unit = {
    classContent.append(
      s"\n\n\n\t//COMPATIBLE - check if this ${pk_table_names._4} is compatible with that ${pk_table_names._3}" +
        s"\n\toverride def compatible(that: ${pk_table_names._4}[Time, PK]): Boolean =" +
  ...
  private def gen_Equals(): Unit = {
    classContent.append(
      s"\n\n\n\t//EQUALS - check if this ${pk_table_names._4} is equal to that ${pk_table_names._4}" +
        s"\n\toverride def equals(that: ${pk_table_names._4}[Time, PK]) =" +
        s"\n\t\t\t\tthis == that"
    )
  }
  ...
  private def gen_Merge(): Unit = {
    classContent.append(
      s"\n\n\t//MERGE this ${pk_table_names._4} with that ${pk_table_names._4}" +
        s"\n\tdef merge(that: ${pk_table_names._4}[Time, PK]) =" +
  ...
  private def gen_Compare(): Unit = {
    classContent.append(
      s"\n\n\t//COMPARE this ${pk_table_names._4} with that ${pk_table_names._4}" +
        s"\n\t//ignore" +
        s"\n\tdef compare(that: ${pk_table_names._4}[Time, PK]) = " +
        s"\n\t\t\t\ttrue"
    )
  }
      ...
      s"\n\n\t//REFERENTIAL INTEGRITY" +
        ...
        fk_Tables.map { ref_table =>
          ...
          if (this_tab_fks.size == 1) { // Single PK » just get that value in this table
            ...
            s"\n\t\t\t\tthis.$ref_tab_name.isVisible( pk )"
            //            new ${ref_table.tableNames._1}_PKs(" +
  ...
  private def gen_Reachable_With_Assumptions(): Unit = {
    ...
    classContent.append(
      s"\n\t\t//HELPER METHOD - ASSUMPTIONS FOR MERGE ASSOCIATIVITY - for the tables of this ${pk_table_names._4}" +
        ...
        s"\n\t\t\tforall(v1: ${pk_table_names._2}[Time, PK], v2: ${pk_table_names._2}[Time, PK], v3: ${pk_table_names._2}[Time, PK])" +
        s"\n\t\t\t\t  { v1.merge(v2).merge(v3) == v1.merge( v2.merge(v3) )" +
        s"\n\t\t\t\t  } &&" +
        fk_Tables_names.map(
          t_Names => {
            s"\n\t\t\tforall(v1: ${t_Names._2}[Time, PK], v2: ${t_Names._2}[Time, PK], v3: ${t_Names._2}[Time, PK])" +
              s"\n\t\t\t\t  { v1.merge(v2).merge(v3) == v1.merge( v2.merge(v3) )" +
              s"\n\t\t\t\t  }"
          }
        ).mkString(" &&") +
  ...
  private def gen_ObjectExtraProofs(): Unit = {
    ...
    classContent.append(
      s"\n\n\n\n/*\n * OBJECT FOR EXTRA PROOFS\n */" +
        s"\nobject ${pk_table_names._4} extends CvRDTProof2[${pk_table_names._4}] {"
    )

    //REF_INTEGRITY
    classContent.append(
      s"\n\n\t//REFERENTIAL INTEGRITY" +
        s"\n\tproof genericReferentialIntegrity[Time, PK] {" +
        s"\n\t\tforall(" +
        s"s1: ${pk_table_names._4}[Time, PK], s2: ${pk_table_names._4}[Time, PK], pk: PK) {" +
    ...
    classContent.append(
      s"\n\n\t//MERGE ASSOCIATIVE" +
        s"\n\tproof mergeIsAssociative[Time, PK] {" +
        s"\n\t\tforall(s1: ${pk_table_names._4}[Time, PK], s2: ${pk_table_names._4}[Time, PK], s3: ${pk_table_names._4}[Time, PK]) {" +
